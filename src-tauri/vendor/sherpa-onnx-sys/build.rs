...
fn download_prebuilt_libs(
    ...
    if lib_dir.is_dir() {
        ...
        if !archive_path.is_file() {
            return Err(format!(
                "Refusing to use cached sherpa-onnx libs without a verifiable archive. \
                 Set SHERPA_ONNX_LIB_DIR to an explicitly trusted lib directory, or provide \
                 {archive_name} through SHERPA_ONNX_ARCHIVE_DIR."
            )
            .into());
        }
    ...
    if !archive_path.is_file() {
        if let Some(local_archive_dir) = env::var_os("SHERPA_ONNX_ARCHIVE_DIR") {
            ...
            verify_archive_checksum(&local_archive_path, &archive_name)?;
            copy_file_atomically(&local_archive_path, &archive_path)?;
        } else {
            expected_archive_sha256(&archive_name)?;
            let version = env!("CARGO_PKG_VERSION");
            let url = format!("{RELEASE_BASE_URL}/v{version}/{archive_name}");
            eprintln!("Downloading verified sherpa-onnx libs from {url}");
...
fn verify_archive_checksum(path: &Path, archive_name: &str) -> Result<(), DynError> {
    let expected = expected_archive_sha256(archive_name)?;
...
fn expected_archive_sha256(archive_name: &str) -> Result<&'static str, DynError> {
    TRUSTED_ARCHIVE_SHA256
        .iter()
        .find_map(|(name, checksum)| (*name == archive_name).then_some(*checksum))
        .ok_or_else(|| {
            format!("No trusted checksum configured for sherpa-onnx archive {archive_name}").into()
        })
}
...
fn archive_name(
    ...
    let name = match (link_mode, target_os, target_arch) {
        ...
        _ => {
            return Err(format!(
                "Just Notes only configures a verified sherpa-onnx prebuilt archive for \
                 macOS arm64 static builds. Set SHERPA_ONNX_LIB_DIR to an explicitly trusted \
                 local lib directory for os={target_os}, arch={target_arch}, link_mode={link_mode:?}."
            )
            .into())
        }
    };
